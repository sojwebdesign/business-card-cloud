...
import { createDeleteLink } from '../../../lib/delete-link';
import { sendDeleteLinkEmail } from '../../../lib/delete-link-email';
import { enforceRateLimit, getClientIp, RateLimitError } from '../../../lib/rate-limit';
import { getPublicOrigin } from '../../../lib/site-url';

export const prerender = false;

const GENERIC_MESSAGE = "If a card exists for this email, we've sent delete confirmation instructions.";
...
export const POST: APIRoute = async ({ request, locals, url }) => {
    try {
        ...
        await enforceRateLimit(kv, 'request-delete-email', email, 5, 3600);
        await enforceRateLimit(kv, 'request-delete-ip', ip, 20, 3600);
        ...
        if (cards.length) {
            ...
            for (const card of cards) {
                const token = await createDeleteLink(kv, card.slug, email);
                payload.push({
            ...
            await sendDeleteLinkEmail(env, origin, email, payload);
        }
        ...
        console.error('Request delete link failed', error);
        return json({ ok: true, message: GENERIC_MESSAGE });
    }
};
